...
Educational antivirus-style scanner using SHA-256 signatures and simple heuristics.
"""
...
# Educational test signature. The EICAR test string is safe and intended for testing scanners.
SIGNATURES = {
    "275a021bbfb6489e54d471899f7db9d1d5e2c0d5c5e0fda07b6f8a6d6f4d8c31": "Example test signature"
}
...
def heuristic_check(path: Path) -> str | None:
    if path.suffix.lower() in SUSPICIOUS_EXTENSIONS:
        return f"Executable or script extension: {path.suffix}"
    try:
        if path.stat().st_size > MAX_SIZE_MB * 1024 * 1024:
            return f"Large file: {path.stat().st_size / (1024 * 1024):.1f} MB"
    except OSError:
    ...
    for index, path in enumerate(paths, start=1):
        ...
        if digest is None:
            ...
            result = ScanResult(
                str(path),
                digest,
                "SUSPICIOUS" if reason else "SAFE",
                reason or "-",
            )
...
class VirusDetectionApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(controls, textvariable=self.path_var, width=70).pack(
        ...
        for column, width in (("file", 430), ("status", 110), ("detail", 220), ("sha256", 180)):
        ...
        self.report_button = ttk.Button(
            bottom, text="Save CSV Report", command=self._save_report, state="disabled"
        )
            ...
            while True:
                ...
                if event[0] == "progress":
                    ...
                    for result in self.results:
                        self.tree.insert(
                            "", "end",
                            values=(result.file, result.status, result.detail, result.sha256),
                        )
                    infected = sum(r.status == "INFECTED" for r in self.results)
                    suspicious = sum(r.status == "SUSPICIOUS" for r in self.results)
                    self.status_var.set(
                        f"Scan complete: {len(self.results)} files | "
                        f"Infected: {infected} | Suspicious: {suspicious}"
                    )
